...
for html_file in html_files:
    ...
    # Fix CSS hrefs using a more aggressive approach that matches relative paths
    content = re.sub(
        r'href="[.\/]*assets/css/simplified/([^"]+)"',
        f'href="{root_prefix}assets/css/simplified/\\1"',
        content
    )
    
    # Fix JS srcs 
    content = re.sub(
        r'src="[.\/]*assets/js/simplified/([^"]+)"',
        f'src="{root_prefix}assets/js/simplified/\\1"',
        content
    )
    
    # Fix direct references to portfolio asset paths
    content = re.sub(
        r'(href|src)="[.\/]*sean-august-horvath-portfolio/assets/',
        f'\\1="{root_prefix}assets/',
        content
    )
